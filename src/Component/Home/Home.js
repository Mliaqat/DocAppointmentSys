...
function Home() {
  ...
  useEffect(() => {
    const getData = async () => {
      const userdata = [];
      await axios
        .get("https://test3-a40a1-default-rtdb.firebaseio.com/doctor.json")
        .then((res) => {
          const data = res.data;
  ...
  return (
    <>
      <div className="container mt-5 mb-5">
        <h1 className="text-center my-5">Doctor List</h1>
                  ...
                  <div className="col-md-3" key={index}>
                    <div className="card p-2 py-3 text-center">
                      <div className="img mb-2">
                        {" "}
                        <img className="img" src={data.image} alt="profile-img" />{" "}
                      </div>
                      <h5 className="mb-0" onClick={() => changeroute(id)}>
                        Dr.{data.name}
                      </h5>{" "}
