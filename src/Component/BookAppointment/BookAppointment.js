...
function BookAppointment() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!appointment.find((obj) => obj.patientEmail === email)) {
      setAlreadyExist(false);   

    await axios
      .post(
        "https://test3-a40a1-default-rtdb.firebaseio.com/appointment.json",
        data
      )
      .then((res) => {
        setgetData(true)
        alert("Your Appointment Added Succfully")
      ...
      <div className="d-flex justify-content-center h-100">
        <div className="card w-75">
          <div className="card-header mt-3">
            ...
            {alreadyExist && <p className="text-danger text-center"> user already exists</p>}
          </div>
